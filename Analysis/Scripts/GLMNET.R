library(glmnet)
M<-read.csv("/home1/02265/viswam78/RScripts/newRScripts/FluxDataThresholdNoTransport1.csv",header=F)
#######
M[abs(M)<0.000001]=0
######
sizeM=dim(M)
...
cvGLMnet_modelCN<-cv.glmnet(as.matrix(Mtrainxx),as.factor(Mtrainyy12),family="multinomial",alpha=1.0,standardize=TRUE,maxit=900000,type.measure="class",nfolds=3,lambda.min.ratio=0.0002,nlambda=2000)
...
###########################
###########################
#GLMnet_modelC<-glmnet(as.matrix(Mtrainxx),as.factor(Mtrainyy1),family="multinomial",alpha=1.0,standardize=FALSE,maxit=9000000,lambda.min.ratio=0.0002,nlambda=2000)

cvGLMnet_modelC<-cv.glmnet(as.matrix(Mtrainxx),as.factor(Mtrainyy1),family="multinomial",alpha=1.0,standardize=TRUE,maxit=9000000,type.measure="class",nfolds=3,lambda.min.ratio=0.0002,nlambda=2000)
...
###########################
#GLMnet_modelN<-glmnet(as.matrix(Mtrainxx),as.factor(Mtrainyy2),family="multinomial",alpha=1.0,standardize=FALSE,maxit=9000000,lambda.min.ratio=0.0002,nlambda=2000)

cvGLMnet_modelN<-cv.glmnet(as.matrix(Mtrainxx),as.factor(Mtrainyy2),family="multinomial",alpha=1.0,standardize=TRUE,maxit=9000000,type.measure="class",nfolds=3,lambda.min.ratio=0.0002,nlambda=2000)
...
pdf(file="/home1/02265/viswam78/RScripts/newRScripts/Noise1Replicate1.pdf")
plot(ClassificationRates)
plot(cvGLMnet_modelCN$cvm)
plot(cvGLMnet_modelC$cvm)
plot(cvGLMnet_modelN$cvm)
dev.off()
